...
export class Game {
  ...
  init(spaceId: string, _overrideServer?: string, _overrideHttpServer?: string) {
    ...
    if (process.env.ENVIRONMENT === "local" || process.env.ENVIRONMENT === "test") {
      ...
      overrideServer = "ws://" + url.hostname + ":8080" + "/__dev-websocket";
    }
  ...
  subscribeToEvent<T extends GameEventCase>(
    ...
    // Having to hard cast these types isn't great and is related to the limitation described below. This is actually an exception to the rule
    // eslint-disable-next-line @typescript-eslint/consistent-type-assertions, @typescript-eslint/no-explicit-any
    ...
    // eventId will always be a *single* type from the GameEventCase union. Given that we know this
    ...
    //@ts-expect-error ts(2532) (no-undefined)
  ...
  triggerItem(closestObject?: string, closestObjectTemplate?: string) {
    this.engine?.sendAction({
      $case: "triggerItem",
      triggerItem: {
        closestObject,
  ...
  interact(objId: string, mapId: string, data?: unknown) {
    this.sendAction({
      $case: "interact",
      interact: {
        objId,
  ...
  getObject(objId: string, mapId?: string) {
    ...
    for (const mapId of mapIds) {
      ...
      for (const key in objects) {
        if (objects[key]?.id === objId) return { mapId: mapId, obj: objects[key] };
      }
    }
    return null;
  }
  ...
  setObject(
    ...
    if (key >= 0) {
      ...
      // in this case, we do NOT want to have a key property on the object in case it was copied from elsewhere
      // because if it has a key property, it will break the array
      delete obj.key;
  ...
  requestAccessViaCheckIn(userId: string, canceled: boolean, name?: string, outfitString?: string) {
    this.engine?.sendAction({
  ...
  EXPERIMENT_enterWithTeammatesInitiateEnterSpace() {
    this.engine?.sendAction({
  ...
  EXPERIMENT_enterWithTeammatesSetOwnLobbyInfo(name: string, outfitString: string) {
    this.engine?.sendAction({
