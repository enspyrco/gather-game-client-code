...
import { fetchGameServerAssignment, SlidingWindow, testConnection } from "./Utils";
...
const RESET_RECONNECT_ATTEMPTS_DELAY = 120000; // have to be connected for 120 seconds to be considered a successful reconnect
...
export class Engine {
  ...
  private async createConnection(reuseServerURL = false) {
    ...
    if (!reuseServerURL || !this.serverURL) {
      const serverURL = await this.getGameServerUrl();
  ...
  private cleanUpAndQueueReconnect(disconnectInfo?: DisconnectInfo) {
    ...
    // if the connection was closed due to a 1006 but we received something from the GS recently then
    // assume that the GS hasn't been reassigned and try connecting to the same GS immediately.
   ...
   */
  private async getGameServerUrl(): Promise<string | null> {
