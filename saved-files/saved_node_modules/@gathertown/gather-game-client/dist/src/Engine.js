...
class Engine {
    ...
    loadUnloadDcState() {
        try {
            ...
            if (!(data === null || data === void 0 ? void 0 : data.timestamp))
                return;
    ...
    get isOnline() {
        var _a, _b;
        return config_1.isBrowser ? (_b = (_a = window === null || window === void 0 ? void 0 : window.navigator) === null || _a === void 0 ? void 0 : _a.onLine) !== null && _b !== void 0 ? _b : true : true;
    }
    createConnection(reuseServerURL = false) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            this.connectionFailedTimeout = setTimeout(() => {
                Logger_1.logger.warn(`t: ${Date.now() - this.connectionInitTime} : didn't get a ws.onopen for ${MAX_WAIT_FOR_CONNECTION}ms after creating connection. destroying and trying again`);
            ...
            if (!reuseServerURL || !this.serverURL) {
                const serverURL = yield this.getGameServerUrl();
                if (!this.started)
                    return;
                if (serverURL) {
            ...
            this.ws.onmessage = (msg) => {
                if (!this.gotFirstMessage) {
                    ...
                    this.onmetric("connection_init_time", Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { space: this.spaceId, attempts: this._reconnectionAttempts, value: connectionInitTime, serverUrl: this.serverURL }));
            ...
            this.ws.onclose = (event) => {
                ...
                this.onmetric(`client_ws_close_code_${event.code}`, Object.assign(Object.assign(Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { value: event.reason, wasClean: event.wasClean, bytesReceivedSinceConnect: this._bytesReceivedSinceConnect, timeSinceConnectionOpened: timeConnectionOpened > 0 ? Date.now() - timeConnectionOpened : null, timeSinceLastSuspend: Date.now() - this.lastSuspendDate, timeSinceLastSignOfLife: Date.now() - this.lastSignOfLife, sizeOfLargestMessageReceived: this.sizeOfLargestMessageReceived }), (this.sizeOfNextLargeMessage && {
                    wasWaitingForMessageOfSize: this.sizeOfNextLargeMessage,
                })), { timeSinceLastSend: Date.now() - this._lastActionSentAt, hasEverFullyConnected: this.hasEverFullyConnected, bufferedAmount: ((_a = this.ws) === null || _a === void 0 ? void 0 : _a.bufferedAmount) || null }));
                this._closeCounts[event.code] = ((_b = this._closeCounts[event.code]) !== null && _b !== void 0 ? _b : 0) + 1;
            ...
            this.ws.onerror = (e) => {
                ...
                this.onmetric(`ws error`, Object.assign(Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { error: e, serverUrl: this.serverURL, timeSinceConnectionOpened: timeConnectionOpened > 0 ? Date.now() - timeConnectionOpened : null, timeSinceLastSuspend: Date.now() - this.lastSuspendDate, timeSinceLastSignOfLife: Date.now() - this.lastSignOfLife, sizeOfLargestMessageReceived: this.sizeOfLargestMessageReceived }), (this.sizeOfNextLargeMessage && {
                    wasWaitingForMessageOfSize: this.sizeOfNextLargeMessage,
                })));
            };
        });
    ...
    destroyConnection(disconnectInfo) {
        ...
        this.cancelGsFetch();
        if (this.ws) {
            this.onmetric(`client_ws_destroy`, Object.assign(Object.assign({}, ((disconnectInfo === null || disconnectInfo === void 0 ? void 0 : disconnectInfo.code) && { closeCode: disconnectInfo === null || disconnectInfo === void 0 ? void 0 : disconnectInfo.code })), { timeSinceLastSignOfLife: Date.now() - this.lastSignOfLife, sizeOfLargestMessageReceived: this.sizeOfLargestMessageReceived }));
            this.ws.onopen = () => { };
            this.ws.onclose = () => { };
            this.ws.onmessage = () => { };
            this.ws.onerror = () => { };
    ...
    cleanUpAndQueueReconnect(disconnectInfo) {
        ...
        const fastReconnect = this.isOnline && this._reconnectionAttempts === 0;
        const reuseServerURL = fastReconnect &&
            this.hasEverFullyConnected &&
            ((disconnectInfo === null || disconnectInfo === void 0 ? void 0 : disconnectInfo.code) === responseCodes_1.GameWsCloseCode.NO_CLOSE_FRAME ||
                ((disconnectInfo === null || disconnectInfo === void 0 ? void 0 : disconnectInfo.code) === responseCodes_1.GameWsCloseCode.GOING_AWAY &&
                    ((_a = disconnectInfo === null || disconnectInfo === void 0 ? void 0 : disconnectInfo.reason) === null || _a === void 0 ? void 0 : _a.toLowerCase().includes("cloudflare")))) &&
            timeSinceLastSignOfLife < REUSE_SERVERURL_1006_SIGN_OF_LIFE_LIMIT;
        this.onmetric("reconnect-attempt", Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { attempts: this._reconnectionAttempts, serverUrl: this.serverURL, hasEverFullyConnected: this.hasEverFullyConnected, timeSinceLastSuccessfulFetch: Date.now() - this.lastSuccessfulFetch, timeSinceLastSignOfLife, timeSinceLastSuspend: Date.now() - this.lastSuspendDate, uninterruptedTimeSinceLastFullSync: uninterruptedLastFullSyncTime > 0 ? Date.now() - uninterruptedLastFullSyncTime : null, fastReconnect,
            reuseServerURL }));
    ...
    startHeartbeat() {
        ...
        this._heartbeatInterval = setInterval(() => {
            ...
            if (this.lastSignOfLife < Date.now() - HEARTBEAT_TIMEOUT && this.ws != null) {
                ...
                this.onmetric("ws-close-timeout", Object.assign({ lastHeartbeat: this.lastSignOfLife, readyState: this.ws.readyState, timeSinceLastSuspend: Date.now() - this.lastSuspendDate, serverUrl: this.serverURL, sizeOfLargestMessageReceived: this.sizeOfLargestMessageReceived }, (this.sizeOfNextLargeMessage && {
                    wasWaitingForMessageOfSize: this.sizeOfNextLargeMessage,
                })));
                this.cleanUpAndQueueReconnect({
    ...
    startRecordingIntentToConnect() {
        const both = () => (Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { serverUrl: this.serverURL, connected: this._connected, online: this.isOnline, timeSinceLastSuspend: Date.now() - this.lastSuspendDate }));
    ...
    trackRefreshFixedConnectionIssue(fixed) {
        if (!this.preRefreshDcData)
            return;
        try {
            this.onmetric("refresh-fixed-connection", Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { value: fixed, history: this.preRefreshDcData.history, attemptsBeforeRefresh: this.preRefreshDcData.attempts, hasEverFullyConnectedBeforeRefresh: this.preRefreshDcData.hasEverFullyConnected, timeSinceDisconnectBeforeRefresh: this.preRefreshDcData.timeSinceDisconnect, timeSinceUnload: Date.now() - this.preRefreshDcData.timestamp, timeSinceStart: Date.now() - this.startTime }));
        }
        catch (_a) {
        }
    ...
    logInitialSyncComplete(isPartialSubscribe) {
        ...
        if (!isPartialSubscribe) {
            ...
            this.onmetric("time-to-sync", Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { hasEverFullyConnected: this.hasEverFullyConnected, timeSinceLastSuspend: Date.now() - this.lastSuspendDate, reconnectAttempts: this._reconnectionAttempts, timeBeforeConnectionInit: this.connectionInitTime -
                    (this.disconnectedStartTime ? this.disconnectedStartTime : this.startTime), timeToGetGameServerUrl: gotGameServerUrl - this.connectionInitTime, timeToOpenWs: gotWsOpen - gotGameServerUrl, timeToGetReady: gotReady - gotWsOpen, timeToGetSubscriptions: gotSubscriptionsUpdated - gotReady, timeToGetMaps: mapDataEndTime - gotSubscriptionsUpdated, timeToGetPlayers: gotSpaceSetsCapacity - mapDataEndTime, timeSinceConnectionInitTime: Date.now() - this.connectionInitTime }));
        }
        this.hasEverFullyConnected = true;
        this.uninterruptedLastFullSyncTime = Date.now();
        if (this.disconnectedStartTime) {
            this.onmetric("dc-length", Object.assign(Object.assign({}, this.getBaseMetricsInfo()), { ms: Date.now() - this.disconnectedStartTime, [`preTimeout${this.LOG_THRESH_MS}`]: !!this.dcLogTimeout, timeSinceLastSuspend: Date.now() - this.lastSuspendDate }));
    ...
    getGameServerUrl() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            try {
                ...
                return yield (0, Utils_1.fetchGameServerAssignment)(source.token, this.spaceId, this.debugOverrideHttpServer);
            }
            catch (e) {
                if (!this.started)
                    return null;
