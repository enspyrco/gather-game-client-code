...
import { numToBase64 } from "./base64";
...
export interface GameMapHttpPublic
  extends Omit<GameMapDB, "objects" | "spaces" | "typeCheckFakeField" | "desks"> {
  objects?: Omit<MapObject, "zIndex">[]; // omit zIndex until later in the NGN-300 migration. backwards compatible rn
  spaces?: Space[];
  desks?: Desks;
...
// when you are adding a new MapObject, x,y, etc are required, but the server will give you a default zIndex (on top of everything else)
...
export function convertWireObjectToMapObjectPartial(
  wireObject: WireObject,
  key: number,
): Partial<MapObject> {
  // this cast is ok because we add the key right after, but it's not ideal
  ...
  const obj = convertWireObjectToMapObjectDBPartial(wireObject) as Partial<MapObjectDB> & {
    key: number;
  };
  obj.key = key;
...
export function convertDesksToDBDesks(desks: Desks): DBDesks {
  ...
  const dbDesks = Object.keys(idsMap).reduce<DBDesks>((dbDesks, deskId) => {
    // @ts-expect-error Error auto-ignored when enabling TS noUncheckedIndexedAccess. If you're already touching this code, please clean this up while you're at it.
    // TODO: @ENG-4257 Clean these up! See the linear task for more context and advice for cleaning up.
...
export const objectsArrayToDict = (
  objects: NonNullable<GameMapHttpPublic["objects"]>,
): NonNullable<GameMapDB["objects"]> => {
  const objOut: GameMapDB["objects"] = {};
  objects.forEach((obj, ind) => {
    objOut[numToBase64(ind)] = { ...obj, zIndex: ind };
  });
  return objOut;
};
export const objectsDictToArray = (
  objects: NonNullable<GameMapDB["objects"]>,
): NonNullable<GameMapHttpPublic["objects"]> =>
  Object.values(objects).sort((a, b) => a.zIndex - b.zIndex);
